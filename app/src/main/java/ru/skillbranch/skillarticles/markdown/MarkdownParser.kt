...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        ...
        var cleanString: StringBuilder = java.lang.StringBuilder()
        parseElements(cleanString, markdownText.elements)
        return cleanString.toString()
    }

    private fun parseElements(str: StringBuilder, elements: List<Element>) {

        for (element in elements) {
            if (element.elements.size <= 1) str.append(element.text)
            else parseElements(str, element.elements)
        }
    }
        ...
        loop@ while (matcher.find(lastStartIndex)) {
            ...
            when (group) {
                ...
                9 -> {
                    ...
                    val (title: String, link: String) = "\\[(.*)\\]\\((.*)\\)".toRegex().find(text, 0)!!.destructured
